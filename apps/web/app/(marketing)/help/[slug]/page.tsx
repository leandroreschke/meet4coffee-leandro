...
export async function generateMetadata({ params }: HelpArticlePageProps): Promise<Metadata> {
  ...
  const article = await getPublishedContentBySlug("help", slug);

  if (!article) {
    return {
      title: `${t("landing.footer.help")} | ${t("app.name")}`,
    };
  }
  ...
  return (
    <main className="mx-auto min-h-screen max-w-4xl px-6 py-12 md:px-8">
      ...
      <article className="surface-card mt-5 rounded-[2.2rem] p-7 md:p-10">
        <p className="text-xs font-semibold uppercase tracking-[0.24em] text-mocha-earth/70">{t("landing.footer.help")}</p>
